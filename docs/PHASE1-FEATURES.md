...
**Status**: ✅ Complete — all three features shipped in v0.1.18 – v0.2.0
**Branch**: merged

> **Historical planning document.** Type signatures below reflect the *original draft*, not the final shipping API. For current API see [`packages/vue-table/API-REFERENCE.md`](../packages/vue-table/API-REFERENCE.md). Notably the final `ServerDataOptions` uses `query` (not `fetch`) — see `src/types.ts:533`.
